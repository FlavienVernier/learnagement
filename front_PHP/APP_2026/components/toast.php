<?php
    ...
    match ($type) {
        'success' => $stylesheet = "bg-success text-on-success",
        'warning' => $stylesheet = "bg-warning text-on-warning",
        default => $stylesheet = "bg-error text-on-error"
    };
?>

<div id="<?= $uuid ?>" class="<?= $stylesheet ?> flex items-center w-full max-w-sm p-4 text-body rounded-lg shadow-xs border border-default" role="alert">
    ...
    <button type="button" class="ms-auto flex items-center justify-center text-body hover:text-heading bg-transparent box-border border border-transparent hover:bg-neutral-secondary-medium focus:ring-4 focus:ring-neutral-tertiary font-medium leading-5 rounded text-sm h-8 w-8 focus:outline-none"
            data-dismiss-target="#<?= $uuid ?>" aria-label="Close">
