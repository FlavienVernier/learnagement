...
function listMAQUETTE_moduleSequencageByIdResp($conn, $id) {
    $sql = "SELECT MAQUETTE_module_sequencage.*, MAQUETTE_module.code_module, LNM_seance_type.type, LNM_groupe_type.groupe_type, ExplicitSecondaryKs_LNM_enseignant.ExplicitSecondaryK as intervenant_principal
...
function checkMAQUETTE_moduleSequencageVsMaquuetteByIdResp($conn, $id) {
    ...
    IFNULL(hCM,0) - IFNULL(hCMCequenced,0) AS 'ecart_CM',  IFNULL(hTD,0)  - IFNULL(hTDCequenced,0) AS 'ecart_TD', IFNULL(hTP,0)  - IFNULL(hTPCequenced,0) AS 'ecart_TP'
...
function checkMAQUETTE_moduleSequencageHVsMaquuetteH($conn) {
    $sql="SELECT 
            `MAQUETTE_module`.`code_module`, 
            IFNULL(`MAQUETTE_module`.`hCM`,0) + IFNULL(`MAQUETTE_module`.`hTD`,0) + IFNULL(`MAQUETTE_module`.`hTP`,0) + IFNULL(`MAQUETTE_module`.`hPROJ`,0) AS `hMCCC`,
            `hCequenced`, 
            ...
            IFNULL(`MAQUETTE_module`.`hCM`,0) + IFNULL(`MAQUETTE_module`.`hTD`,0) + IFNULL(`MAQUETTE_module`.`hTP`,0) + IFNULL(`MAQUETTE_module`.`hPROJ`,0) - `hCequenced` AS `Reste à planifier`
        FROM `MAQUETTE_module`
        JOIN (
    ...
    MAQUETTE_module.id_module, 
    MAQUETTE_module.code_module, 
    IFNULL(hCM,0) - IFNULL(hCMCequenced,0) AS 'ecart_CM',  IFNULL(hTD,0)  - IFNULL(hTDCequenced,0) AS 'ecart_TD', IFNULL(hTP,0)  - IFNULL(hTPCequenced,0) AS 'ecart_TP',  IFNULL(hPROJ,0) - IFNULL(hPROJCequenced,0) AS 'ecart_PROJ'
FROM MAQUETTE_module
LEFT JOIN (
    SELECT code_module, SUM(nombre * duree_h) as hCMCequenced
    FROM MAQUETTE_module_sequencage 
