<?php
    function select_charge_totale($conn, $user_id){
        $sql = "
            SELECT SUM(CASE
            WHEN LNM_seance_type.type = 'CM' THEN MAQUETTE_module_sequencage.duree_h
            WHEN LNM_seance_type.type = 'TD' THEN MAQUETTE_module_sequencage.duree_h
            WHEN LNM_seance_type.type = 'TP' THEN MAQUETTE_module_sequencage.duree_h
            END) AS 'Charge'
    ...
    function select_charge_repart($conn, $user_id){
        $sql = "
            SELECT 
                SUM(case when LNM_seance_type.type = 'CM' THEN MAQUETTE_module_sequencage.duree_h END) AS 'CM',
                SUM(case when LNM_seance_type.type = 'TD' THEN MAQUETTE_module_sequencage.duree_h END) AS 'TD',
                SUM(case when LNM_seance_type.type = 'TP' THEN MAQUETTE_module_sequencage.duree_h END) AS 'TP'
